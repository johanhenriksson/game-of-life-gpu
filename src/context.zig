...
const InstanceDispatch = vk.InstanceWrapper(&[_]vk.ApiInfo{
    .{
        .instance_commands = .{
...
const DeviceDispatch = vk.DeviceWrapper(&[_]vk.ApiInfo{
    .{
        .device_commands = .{
            .destroyDevice = true,
            .getDeviceQueue = true,
...
pub const VkContext = struct {
    ...
    pub fn init(allocator: Allocator, app_name: [*:0]const u8, window: sdl.Window) !VkContext {
        ...
        const sdl_exts = try sdl.vulkan.getInstanceExtensionsAlloc(window, allocator);

        var instance_extensions = try std.ArrayList([*:0]const u8).initCapacity(allocator, 1);
        try instance_extensions.appendSlice(sdl_exts);
